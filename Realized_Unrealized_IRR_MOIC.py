...
ws['R1'] = 'Realized IRR'
ws['S1'] = 'Realized MOIC'
ws['T1'] = 'Unrealized IRR'
ws['U1'] = 'Unrealized MOIC'
for col in ['R1', 'S1', 'T1', 'U1']:
...
for row in range(2, ws.max_row + 1):
    ...
    if vcusip in pos_cusips:
        ws.cell(row=row, column=20, value=irr if irr is not None else "NA")
        ws.cell(row=row, column=21, value=m if m is not None else "NA")
    ...
    #Realized
        ...
        ws.cell(row=row, column=20, value="NA")
        ws.cell(row=row, column=21, value="NA")
